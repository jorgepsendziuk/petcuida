...
serve(async (req) => {
  ...
  try {
    const { imageBase64, petId, userId }: OCRRequest = await req.json();

    if (!imageBase64 || !petId || !userId) {
      return new Response(
        JSON.stringify({ error: "imageBase64, petId e userId são obrigatórios" }),
    ...
    if (petError || !pet) {
      return new Response(
        JSON.stringify({ error: "Pet não encontrado ou não pertence ao usuário" }),
...
Se algum campo não estiver visível ou não puder ser determinado, use null.`;
    ...
    const openaiResponse = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o", // ou "gpt-4-vision-preview" se disponível
        messages: [
          {
            role: "system",
    ...
    const treatmentsToInsert = prescriptionData.medications.map((med) => {
